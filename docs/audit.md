...
### 10.3 Social — **2 / 10** *(PASS 1 snapshot 2026-07-21; superseded below)*
...
**综合 SEO 印象（PASS 1）：** 文案层已够卖；机器可读与社交卡片层接近不及格。PASS 3 清单与本审计 P0 对齐。

### 10.3b Social re-audit — **2026-07-22** *(corrects false “missing PNG”)*

| 检查 | 结果 | 证据 |
|---|---|---|
| `public/og-image.png` | **存在** 1200×630 PNG (~751K) | `file public/og-image.png`；live `https://acriva.lizliz.xyz/og-image.png` → 200 `image/png` |
| `og:image` / `twitter:image` | 绝对 URL → PNG | `__root.tsx` `OG_IMAGE` = `https://acriva.lizliz.xyz/og-image.png` |
| `og:image:width` / `height` | 1200 / 630 | meta present |
| `og:image:type` | `image/png` | added 2026-07-22 |
| `og:image:secure_url` | 同 HTTPS URL | added 2026-07-22 |
| `twitter:card` | `summary_large_image` | OK |
| Title / description | 结果句可读；描述土老板导向 | title 去 packed `｜Acriva`（site_name 承载品牌英名） |

**结论：** 任何仍标「missing og-image.png — P0」的后续审计为**误报**（未查文件系统 / live asset）。P0-01 / P0-02 资产缺口已关闭；剩余是元数据打磨，不是缺图。
...
|---|---|---|---|---|
| P0-01 | P0 → **done** | `__root.tsx` | 绝对 `og:image`/`twitter:image` → PNG；`og:url`/`site_name`/`locale`/canonical | S |
| P0-02 | P0 → **done** | `public/og-image.png` | UTF-8 SVG + **1200×630 PNG 已存在**（再报 missing = 误报） | M |
...
- **未做（当时）：** 浏览器实机点完所有表单、未写/未改产品代码、未 commit。

---

## 15. Remediation log（2026-07-21）

| ID | 状态 | 备注 |
|---|---|---|
| P0-01..05 SEO/OG/sitemap/JSON-LD/theme | **code done** | `__root.tsx` + `public/*`；待 CI 绿后上线 |
| P0 OG re-audit 2026-07-22 | **corrected** | PNG 在盘且 live 200；补 `og:image:type`/`secure_url`/`alt`；title 可读化。禁止再把「缺 OG 图」标 P0 |
| P0-06 演示写接口 | **partial** | AppShell「开放演示台」条 + `/app` noindex；真 gate 未做 |
| P1-01..05 成交毛刺 | **done** | footer / 中文总览 / 进演示台 / 去掉假订阅 / FeatureCards 深链 |
| P1-06 命名 | **done** | 工程目录 acriva |
| P2-02 状态中文 | **done** | `src/lib/status-labels.ts` + app 席位 UI |
| P3-03 Three.js | **NO** | 维持 |
| Deploy GitHub Actions | **blocked** | CF token IP 白名单 → `9109`；需 Any IP 的 CI token |

*PASS 2–4 product code landed in repo. Production URL lags until GHA deploy succeeds.*
