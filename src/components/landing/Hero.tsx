...
const feedEn = [
  ...
  { title: "Booked: greenhouse visit", meta: "Tue 09:00 · Prof. Lin Wei", tone: "violet" as const },
  { title: "Buyer demand · apple", meta: "CityFresh · awaiting", tone: "slate" as const },
    ...
    >
      <div className="space-y-2 p-3 pb-8">
        <div className="flex items-center justify-between rounded-lg border border-[#E8DFD0] bg-[#F7F0E4] px-2.5 py-2 text-[12px]">
          <span className="text-[#6F6558]">{c.sampleLoan}</span>
          <span className="badge badge-warn">{c.applying}</span>
        </div>
        {(
          [
            [c.purpose, c.samplePurpose],
            [c.amount, "¥480,000"],
            [c.region, c.sampleRegion],
          ] as const
        ).map(([label, value]) => (
          <div
            key={label}
      ...
      <AuditTrailStrip batch="#A-2607" time="14:32" operator={c.auditFarmer} />
    </WindowChrome>
  );
}
...
function BookWindow() {
  ...
  const reduceMotion = useReducedMotion();
  return (
    <WindowChrome title={c.bookTitle} className="interactive-window">
      <div className="p-3 pb-8">
        <div className="mb-2 text-[12px] font-medium text-[#1C1712]">{c.bookTopic}</div>
        <div className="mb-3 grid grid-cols-2 gap-2 text-[11px]">
          <div className="rounded-lg bg-[#F7F0E4] px-2.5 py-2 text-[#4A433A]">{c.bookTime1}</div>
          <div className="rounded-lg bg-[#F7F0E4] px-2.5 py-2 text-[#4A433A]">{c.bookTime2}</div>
        </div>
        <div className="mb-3 flex items-center gap-2">
          <Avatar initials="L" tone="emerald" size="sm" />
          <div className="text-[12px]">
            <div className="font-medium text-[#1C1712]">{c.expertName}</div>
            <div className="text-[#6F6558]">{c.expertTitle}</div>
          </div>
        </div>
        <button
          type="button"
          ...
          onClick={(e) => {
            ...
            el.textContent = c.confirmed;
            if (reduceMotion) return;
            window.setTimeout(() => {
              el.textContent = c.confirmBook;
            }, 1600);
          }}
        >
          {c.confirmBook}
      ...
      <AuditTrailStrip batch="#B-0719" time="09:12" operator={c.auditExpert} />
    </WindowChrome>
  );
}
